...
WHERE ISBN IN
      (SELECT ISBN FROM ORDERITEMS HAVING SUM(QUANTITY) =
...
FROM BOOKS
WHERE CATEGORY IN
      (SELECT DISTINCT CATEGORY FROM BOOKS NATURAL JOIN ORDERITEMS NATURAL JOIN ORDERS WHERE CUSTOMER# = 1007)
AND ISBN NOT IN (SELECT ISBN FROM ORDERITEMS NATURAL JOIN ORDERS WHERE CUSTOMER# = 1007);



SELECT SHIPCITY, SHIPSTATE FROM ORDERS WHERE (SHIPDATE-ORDERDATE) =
...
FROM ORDERS NATURAL JOIN ORDERITEMS
WHERE ISBN IN (SELECT ISBN FROM BOOKAUTHOR NATURAL JOIN AUTHOR WHERE FNAME = 'JAMES' AND LNAME = 'AUSTIN');
...
FROM (SELECT ORDER#, NVL(SHIPCOST, 0) SHIPCOST FROM ORDERS) NATURAL JOIN
